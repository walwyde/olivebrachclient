...
export const saveMessage = (data, id) => async (dispatch) => {
  try {
    ...
    const res = await axios.post(`/api/conversations/${id}/messages`, data);
...
export const deleteMessage =
  (conversationId, messageId) => async (dispatch) => {
    try {
      const res = await axios.post("/api/conversations/delete-message", {
        conversationId,
        messageId,
      });
      if (res.status === 200) {
...
export const initConversation = (id) => async (dispatch) => {
  try {
    ...
    const res = await axios.post(`/api/conversations/${id}`, { _id: id });
    if (res.data) {
      dispatch({
        type: init_convo,
        payload: res.data,
...
export const deleteConvo = (convoId) => async (dispatch) => {
  try {
    const res = await axios.post("/api/conversations/delete-conversation", {
      convoId,
    });
...
export const loadNewMessages = (userId) => async (dispatch) => {
  try {
    const res = await axios.get(`/api/conversations/new-messages`);
